 ...
 * @since   0.0.1
 */
class Minstagram_Tags_Widget extends WP_Widget {

	function __construct() {
		require_once( dirname( __FILE__ ) . '/class.minstagram-widget-model.php' );
		parent::__construct(
			false,
			__( 'MinStagram Tags Wieget', 'minsta' ),
			array(
				'description' => __( 'Shows Instagram images specified in the hash tag.', 'minsta' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		$items = Minstagram_Widget_Model::get( $instance, 'tags' );
	 ...
	 * Required input is minsta_tag, minsta_client_id, minsta_count.  
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( '' !== $new_instance['minsta_tag'] ) {
			set_transient( 'minsta_tag_error', false, 10 );
			$instance['minsta_tag'] = $new_instance['minsta_tag'];
		} else {
			set_transient( 'minsta_tag_error', true, 10 );
		}
	...
	function form( $instance ) {
		...
		'minsta_tag'             => isset( $instance['minsta_tag'] ) ? $instance['minsta_tag'] : '',
		...
		// On error, Set true.
		'minsta_tag_error'       => get_transient( 'minsta_tag_error' ),
